...
def parse_single_population(text: str) -> Dict[int, Dict[float, int]]:
    ...
    """
    # If the run did not terminate normally, avoid attempting to parse
    # population tables — return empty results so callers can handle
    # incomplete/failed runs gracefully.
    if not terminated_normally(text):
        return {}
...
def parse_transitions_and_rates(text: str) -> Tuple[Dict[int, Dict[float, int]], Dict[int, Dict[float, float]]]:
    ...
    """
    # If the run did not terminate normally, do not attempt to extract
    # transitions / rates — return empty mappings.
    if not terminated_normally(text):
        return {}, {}
